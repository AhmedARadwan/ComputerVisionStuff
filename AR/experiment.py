...
def part_3():
    ...
    # input_images = ['ps3-3-b_base.jpg']
    # output_images = ['ps3-3-a-2.png']
...
def part_7():
    ...
    #            [115, 144],
    #            [ 93, 212]]
    # np.save('p1.npy', points1)
    # np.save('p2.npy', points2)
...
def part_8(path3):
    ...
    # image = cv2.imread("./input_images/ps3-8-1.png", 1)
    image_gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY).astype(np.float64)

    automaticCornerDetection = Automatic_Corner_Detection()
    # Ix, Iy = automaticCornerDetection.gradients(image_gray)
    ...
    # sx2, sy2, sxsy = automaticCornerDetection.second_moments(image_gray, 7, 10)
    # fig, axs = plt.subplots(1, 4, figsize=(15, 15))
    # axs[0].imshow(image_gray, cmap='gray')
    # axs[0].set_title('Original Image')
    # axs[1].imshow(sx2, cmap='gray')
    # axs[1].set_title('Sx2')
    # axs[2].imshow(sy2, cmap='gray')
    # axs[2].set_title('Sy2')
    # axs[3].imshow(sxsy, cmap='gray')
    # axs[3].set_title('SxSy')
    # plt.show()

    # responseMap = automaticCornerDetection.harris_response_map(image_gray, 7, 5, 0.04)
    # # print(image_gray)
    # fig, axs = plt.subplots(1, 2, figsize=(15, 15))
    # axs[0].imshow(image_gray, cmap='gray')
    # axs[0].set_title('Original Image')
    # axs[1].imshow(responseMap, cmap='gray')
    # axs[1].set_title('Harris Response Map')
    # plt.show()

    # cv2.namedWindow("Response", cv2.WINDOW_NORMAL)
    # cv2.imshow("Response", responseMap)
    # cv2.waitKey(0)

    x_corners, y_corners = automaticCornerDetection.harris_corner(image_gray, 100)
    img_corners = image.copy()
    for x, y in zip(x_corners, y_corners):
        cv2.circle(img_corners, (x, y), 10, (0, 0, 255), -1)

    cv2.imwrite('ps3-8-2.jpg', img_corners)


    fig, axs = plt.subplots(1, 2, figsize=(15, 15))
    axs[0].imshow(image_gray, cmap='gray')
    axs[0].set_title('Original Image')
    axs[1].imshow(img_corners, cmap='gray')
...
def part_9_a(homography_parameters, path1, path2):
    ...
    cv2.imwrite(IMG_DIR + '/ps3-9-1.jpg', im_warped)
    ...
    cv2.imwrite(IMG_DIR + '/ps3-9-2.jpg', im_out_mosaic)
    cv2.imshow("Output Mosaic Image", im_out_mosaic)

    cv2.waitKey(0)


def show_manual_points(image1_path, image2_path, points1_path, points2_path):
    ...
    for point in points1:
        cv2.circle(image1, (point[0], point[1]), 5, (0, 0, 0), -1)
        cv2.putText(image1, f'({point[0]},{point[1]})', (point[0], point[1]-20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)

    for point in points2:
        cv2.circle(image2, (point[0], point[1]), 5, (0, 0, 0), -1)
        cv2.putText(image2, f'({point[0]},{point[1]})', (point[0], point[1]-20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
...
if __name__ == '__main__':
    ...
    # for the report :)
    # show_manual_points(path1, path2, 'p1-Manual.npy', 'p2-Manual.npy')

    # Part 7
    homography_parameters = part_7()

    path3 = os.path.join(IMG_DIR, "ps3-8-1.png")
    # Part 8
    part_8(path3)
