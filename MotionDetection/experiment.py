...
def part_1a():
    ...
    k_size = 9              # Select a kernel size
    k_type = "uniform"     # Select a kernel type
    sigma = 0.3               # Select a sigma value if you are using a gaussian kernel
    ...
    k_type = "gaussian"     # Select a kernel type
    sigma = 3.0               # Select a sigma value if you are using a gaussian kernel
...
def part_1b():
    ...
    k_size = 15              # Select a kernel size
    k_type = "gaussian"     # Select a kernel type
    sigma = 10               # Select a sigma value if you are using a gaussian kernel
...
def part_5a():
    ...
    out_images.append(shift_0)

    # get flow from shift_0 to shift_r10
    levels = 4  # Define the number of levels
    k_size = 3  # Select a kernel size
    k_type = "uniform"  # Select a kernel type
    ...
    # warp using 0.2, 0.4, 0.6, 0.8
    ranges = [0.2, 0.4, 0.6, 0.8]
    for range in ranges:
        out_img = ps4.warp(shift_0, u*range, v*range, interpolation, border_mode)
        out_images.append(out_img)

    out_images.append(shift_r10)


    row1 = np.hstack((out_images[0:3]))
    row2 = np.hstack((out_images[3:6]))
    out_img = np.vstack((row1, row2))
    cv2.imwrite(os.path.join(output_dir, "ps4-5-a-1.png"), ps4.normalize_and_scale(out_img))
...
def part_5b():
    ...
    mc1 = cv2.imread(os.path.join(input_dir, 'MiniCooper', 'mc01.png'),
                              0) / 255.
    mc2 = cv2.imread(os.path.join(input_dir, 'MiniCooper', 'mc02.png'),
                              0) / 255.
    
    out_images.append(mc1)

    # get flow from shift_0 to shift_r10
    levels = 9  # Define the number of levels
    k_size = 9  # Select a kernel size
    k_type = "uniform"  # Select a kernel type
    sigma = 0  # Select a sigma value if you are using a gaussian kernel
    interpolation = cv2.INTER_CUBIC  # You may try different values
    ...
    u, v = ps4.hierarchical_lk(mc1, mc2, levels, k_size,
                               k_type, sigma, interpolation, border_mode)
    
    # cv2.imshow("uv plot", quiver(u, v, scale=3, stride=10))
    # cv2.waitKey(0)
    ...
    ranges = [0.2, 0.4, 0.6, 0.8]
    for range in ranges:
        out_img = ps4.warp(mc1, -1*u*range, -1*v*range, interpolation, border_mode)
        cv2.imwrite(os.path.join(output_dir, "ps4-5-b-1-{}.png".format(range)), ps4.normalize_and_scale(out_img))
        out_images.append(out_img)

    out_images.append(mc2)


    row1 = np.hstack((out_images[0:3]))
    row2 = np.hstack((out_images[3:6]))
    out_img = np.vstack((row1, row2))
    cv2.imwrite(os.path.join(output_dir, "ps4-5-b-1.png"), ps4.normalize_and_scale(out_img))
    ...
    out_images2 = []
    mc3 = cv2.imread(os.path.join(input_dir, 'MiniCooper', 'mc03.png'),
                              0) / 255.
    # mc3 = cv2.cvtColor(mc3, cv2.COLOR_BGR2GRAY)
    out_images2.append(mc2)
    ...
    u2, v2 = ps4.hierarchical_lk(mc2, mc3, levels, k_size,
                               k_type, sigma, interpolation, border_mode)

    # cv2.imshow("uv plot", quiver(u2, v2, scale=3, stride=10))
    # cv2.waitKey(0)

    # warp using 0.2, 0.4, 0.6, 0.8
    ranges = [0.2, 0.4, 0.6, 0.8]
    for range in ranges:
        out_img = ps4.warp(mc2, -1*u2*range, -1*v2*range, interpolation, border_mode)
        cv2.imwrite(os.path.join(output_dir, "ps4-5-b-2-{}.png".format(range)), ps4.normalize_and_scale(out_img))
        out_images2.append(out_img)

    out_images2.append(mc3)


    row1 = np.hstack((out_images2[0:3]))
    row2 = np.hstack((out_images2[3:6]))
    out_img = np.vstack((row1, row2))
    cv2.imwrite(os.path.join(output_dir, "ps4-5-b-2.png"), ps4.normalize_and_scale(out_img))
...
def part_6():
    ...
    frames = ps4.read_video(os.path.join(input_dir, 'videos', 'person10_handclapping_d4_uncomp.avi'))
    classification = ps4.classify_video(frames)
    print("Classification: ", classification_to_string[classification])
